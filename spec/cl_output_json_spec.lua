...
describe("Tests the busted json output", function()
  ...
  it("notifies with error if results cannot be encoded", function()
    local success, exit_code, out, err = execute(busted_cmd .. ' ' .. "--pattern=cl_output_json.lua$ --output=busted/outputHandlers/json.lua")
  ...
  pending("encodes non-string resuls", function()
    -- code
  end)
end)
